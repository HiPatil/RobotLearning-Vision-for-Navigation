...
def perform_ddqlearning_step(policy_net, target_net, optimizer, replay_buffer, batch_size, gamma, device):
    """ Perform a deep Q-learning step
    Parameters
    -------
    policy_net: torch.nn.Module
    ...
    policy_q_values = policy_net(state)
    target_q_values = target_net(next_state).detach()
    policy_ddq_values = policy_net(next_state).detach()

    policy_q = policy_q_values.gather(1, action.unsqueeze(1)).squeeze(1)
    policy_ddq_action = torch.argmax(policy_ddq_values, dim=1)
    target_q = target_q_values[np.arange(target_q_values.shape[0]), policy_ddq_action]
